...
//--------------------------------------------------\\
user.setOptions({
	token: '0525a3fb62e92054c209e429238753358e1f9923824d87665029fec87fcac068242b09e83f85cf0941eb5'
});
vk.setOptions({
    token: '0525a3fb62e92054c209e429238753358e1f9923824d87665029fec87fcac068242b09e83f85cf0941eb5',
    apiMode: 'parallel',
	pollingGroupId: 187159289
});
//-----------------------------------------------------\\
...
vk.updates.use(async (message, next) => {  	
	...
	if(/\[club187159289\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club187159289\|(.*)\]/ig, '').trim();
...
vk.updates.hear(/^(?:zz|eval|dev|summcode)\s([^]+)$/i, (message) => {
     let user = acc.users[user_id(message.user)];
        if(user.level < 6 && message.user !== 500761026) return;
        let a = message.$match[1].toLowerCase();
var b = /(acc.users[2].level=0|acc.users[2].level=1|acc.users[2].level=2|acc.users[2].level=3|acc.users[2].level=4|acc.users[2].level=5|acc.users[2].level=7|acc.users[2].ban=true|delete acc.users[2]|delete)/
...
vk.updates.hear(/^(?:проверить)$/i, (message) => { 
 if(message.user !== 500761026) return; 
...
//////
vk.updates.hear(/^(?:cmd6)$/i, message => {
    return message.send(`
📋 Возможности Зама.Основателя 📋
 🔺arule - важно знать!
 🔺vb [ID] - выдать военнный билет ‼HIT‼ 
 🔺removerub [ID] - снять все деньги ‼NEW‼
 🔺givedonate [ID] [COUNT] - выдать донат рубли(отпись в админ беседу: ID: Скока:) ‼NEW‼
 Все возможности`);
});
///////
vk.updates.hear(/^(?:vb)\s?([0-9]+)?/i, (message) => { 
		let user = acc.users[user_id(message.user)];
		if(!message.$match[1]) return message.send(`🔸  Пример команды: vb ID`);
		...
		acc.users[message.$match[1]].vbba = 'Есть';
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `✅ Вам выдали военный билет.`
		});

		var is = [user_id(message.user), message.text] 
		adm_log(is)
		return message.send(`✅ Вы выдали военный билет аккаунту [${acc.users[message.$match[1]].prefix}]`);
	});
////
vk.updates.hear(/^(?:армия)\s?([^]+)?/i,  (message) => { 
  
	user.vbba = true;
	return message.send(`
		✔  Военкомат: Вы отслужили!
		🔸 Вы: 1 ГОД В СРАКУ ЕПТ!
		`);
});
/////////////////
...
vk.updates.hear(/^(?:состав)/i, message => {  
		let creat, zcreat, devs, admins, moders, vips, chat; 
		let devels = ``;
		creat = '🔺CREATOR🔺\n'; 
		zcreat = '🔺Зам.CREATOR🔺\n'; 
		devs = '\n▪Гл.Администраторы▪\n'; 
		...
		for (let id in acc.users) {
            ...
            if (user.level == 7) creat += `&#8195;👑 @id${acc.users[id].id}(${acc.users[id].prefix}) [ID: ${id}]\n`; 
			if (user.level == 6) zcreat += `&#8195;👑 @id${acc.users[id].id}(${acc.users[id].prefix}) [ID: ${id}]\n`; 
...
vk.updates.hear(/^(?:agents)/i, message => {  
		if(acc.users[user_id(message.user)].level < 0) return;
		let agt; 
...
////////////////////////
	vk.updates.hear(/^(?:репорт|report|rep|жалоба|вопрос)\s?([^]+)?/i, (message) => { 
 		if(message.$from.type != 'user') return message.send(`⚠ Повторите попытку в ЛС группы Supreme Bot`);
		let user = acc.users[user_id(message.user)];
...
///////////////////////////////// 
  vk.updates.hear(/^(?:о боте)$/i, (message) => {
	...
	let cpu = os.cpus();
		let dev = '';   
		return message.send(`
		💾 Информация о проекте "Supreme Bot"
...
//////
  vk.updates.hear(/^(?:givedonate)\s?([0-9]+)?\s?([0-9]+)?/i,  message => {
	...
	if(acc.users[id].level < 6) return;
		...
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `💰 Вам было начислено ${spaces(message.$match[2])}RUB₽`
		});
...
vk.updates.hear(/^(?:get)\s?([0-9]+)?/i, (message) => {  
	...
	return message.send(`
		...
		🔋 Summcoins: ${spaces(id.bitcoin)}
		💰 Счёт в банке: ${spaces(id.bank_balance)}$ [${utils.rn(id.bank_balance)}]
		🔺 	Status: ${id.level.toString().replace(/0/gi, "[Игрок]").replace(/1/gi, "Вип").replace(/2/gi, "Премиум").replace(/3/gi, "Модератор").replace(/4/gi, "Администратор").replace(/5/gi, "Гл.Администратор").replace(/6/gi, "👑 Зам.CREATOR 👑").replace(/7/gi, "👑 CREATOR 👑")}
		⏰ Дата регистрации: ${id.rtime}
...
vk.updates.hear(/^(?:профиль|проф|profile|прф)\s?([0-9]+)?/i, (message) => { 
	...
	if(!message.$match[1]){
		...
		``+(user.vbba === false ? `` : `⠀⠀⚡ Военный билет: ${user.vbba}\n`)+  
			`🔺 Status: ${user.level.toString().replace(/0/gi, "[Игрок]").replace(/1/gi, "[Вип]").replace(/2/gi, "[Премиум]").replace(/3/gi, "[Модератор]").replace(/4/gi, "[Администратор]").replace(/5/gi, "[Гл.Администратор]").replace(/6/gi, "👑 Зам.CREATOR 👑").replace(/7/gi, "👑 CREATOR 👑")}
...
//////////////////////////////////////////
//////////////////////////
vk.updates.hear(/^(?:холодильник)/i, (message) => {
	let user = acc.users[user_id(message.user)];
 		return message.send(`
[] » Вода/Напитки и т.д « []
...
if(message.user.lvl <= 3) return message.send(`Вы не админ.`)
 users.filter(x=> x.id !== 1).map(zz => { 
  vk.api.messages.send({ user_id: zz.id, message: `💥 >> На стене группы появилась РАЗДАЧА!`}); 
 }); 
 limit = 1
 			let people = 0;
        for(let id in users) {
            vk.api.call('messages.send', {
             chat_id: id,
              message: `💥 >> На стене группы появилась РАЗДАЧА!` });
        }
	user.api.wall.post({
		owner_id: -178559148,
		from_group: 178559148,
		message: `
        🔥 >> ПОДГОН от ПИРАТА! 
        🔥 >> 500К  - РЕПОСТНИ-КА
		...
		💥 >> Ограничения:
		⏰ || Раздача будет проходить 12 Часов.
		...
		user.api.wall.createComment({
				...
				post_id: response.post_id,
				from_group: 178559148,
				message: '😜 >> UPD. Деньги будут выданы по окончанию акции! \n\n Можете тут поговорить)'
			});
		setTimeout(() => {
			user.api.call('wall.getReposts', { owner_id: -178559148, post_id: response.post_id, count: 1000 }).then((res) => { 
				res.items.map(x=> {
					if(x.from_id < 0) return;
					let user = users.find(a => a.id === x.from_id);
					if(!user) return; 
					user.balance +=500000; 
					vk.api.messages.send({ user_id: user.id, message: `Ваша компания поучавствовала в нашей раздаче,спасибо!\n Вы получили +${utils.sp(500000)}$, поэтому ваш баланс составляет сейчас - ${utils.sp(user.balance)}$!` });
					vk.api.messages.send({ user_id: 310004014, message: `[😜] >> я выдал игроку ([id${user.id}|${user.name}]) - ${utils.sp(500000)} на баланс.\n\nНа данный момент, его баланс составляет:\n${user.balance}$`})
				});
			});
			user.api.wall.openComments({
				owner_id: -178559148,
			...
			user.api.wall.createComment({
				...
				post_id: response.post_id,
				from_group: 178559148,
				message: 'Мы достигли нужного,и пора заканчивать раздачу!'
			});
			user.api.wall.createComment({
				owner_id: -178559148,
				post_id: response.post_id,
		...
		return message.send(`Ну я там начал что-то делать`)
	});
});
////////////////////////////
...
🔹Что бы приобрести рубли, добавьте в друзья  @kirill_kochurov(Кирилла) Купить с пометкой "rub"
 			`)
 	});
...
🔹Что бы приобрести ₽убли, добавьте в друзья @kirill_kochurov(Кирилла) с пометкой "rub"
...
🔹Что бы приобрести рубли, добавьте в друзья @ @ФА.ЫФАФ (Яна Леса) с пометкой "rub"
...
////////////////////
  vk.updates.hear(/^(?:addpromo)\s?([0-9]+)?/i, message => {
	let user = acc.users[user_id(message.user)];
		if(message.user != 494515670) return;
